...
class Graph:
    def __init__(self, n, edges):
        ...
        for edge in edges:
            ...
            self.indegree[u] += 1
            self.indegree[v] += 1
        self.leafs = [u for u in self.indegree if self.indegree[u] == 1]
...
class Solution:
    def minimumSemesters(self, n: int, edges: List[List[int]]) -> List[int]:
        if n == 1:
            return [0]
        graph = Graph(n, edges)
        semester_count = 0
        while graph.leaf_count() > 1:
            graph.remove_leaf()
            semester_count += 1
        print(semester_count, graph.nodes, graph.leafs)
        return semester_count + 1 if graph.leaf_count() == graph.node_count() else -1


# print(Solution().minimumSemesters(3, [[1, 3], [2, 3]]))
# print(Solution().minimumSemesters(5, [[1, 3], [2, 3], [4, 1], [5, 2]]))
# print(Solution().minimumSemesters(3, [[1, 2], [2, 3], [3, 1]]))
# print(Solution().minimumSemesters(
